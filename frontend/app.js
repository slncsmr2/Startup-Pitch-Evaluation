...
const videoSelectorGroup = document.getElementById("videoSelectorGroup");
...
function handleVideoUpload(event) {
  ...
  // Clear the dropdown since we have an uploaded file
  videoSelect.value = "";
  videoSelectorGroup.classList.add("hidden");

  // Load the video
  pitchVideo.src = uploadedVideoObjectUrl;
...
async function loadVideoList() {
  try {
    ...
    // Show video selector group if there are videos
    videoSelectorGroup.classList.remove("hidden");
  } catch (err) {
    console.error("Failed to load videos:", err);
    // Hide selector if no videos available
    videoSelectorGroup.classList.add("hidden");
  }
}
